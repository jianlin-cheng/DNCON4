...
def _conv_bn_relu2D(filters,  nb_row, nb_col, subsample=(1, 1)):
    def f(input):
        conv = Conv2D(filters=filters, kernel_size=(nb_row, nb_col), strides=subsample,
...
# Helper to build a conv -> BN -> relu block
def _conv_relu2D(filters, nb_row, nb_col, subsample=(1, 1)):
    def f(input):
        conv = Conv2D(filters=filters, kernel_size=(nb_row, nb_col), strides=subsample,
                             kernel_initializer="he_normal", padding="same")(input)
        # norm = BatchNormalization(axis=1)(conv)
...
# Helper to build a conv -> BN -> softmax block
def _conv_bn_sigmoid2D(filters, nb_row, nb_col, subsample=(1, 1)):
    def f(input):
        conv = Conv2D(filters=filters, kernel_size=(nb_row, nb_col), strides=subsample,
                             kernel_initializer="he_normal", padding="same")(input)
        norm = BatchNormalization(axis=-1)(conv)
...
#     print("Setting hidden models as ",hidden_type)
#     ########################################## set up 1D model
#     DNCON4_1D_input_shape =(None,feature_1D_num)
#     filter_sizes=win_array
#     DNCON4_1D_input = Input(shape=DNCON4_1D_input_shape)
#     DNCON4_1D_convs = []
#     for fsz in filter_sizes:
...
#     #ss_model.summary()
#     ## convert 1D to 2D
#     print(DNCON4_1D_out.shape)
#     DNCON4_genP=generatePairwiseF(output_shape=(DNCON4_1D_out.shape.as_list()[1],DNCON4_1D_out.shape.as_list()[1],DNCON4_1D_out.shape.as_list()[2]*3),batch_size=batch_size)(DNCON4_1D_out)
    
    
#     # load 2D contact features
#     contact_feature_num_2D=feature_2D_num
#     contact_input_shape=(DNCON4_1D_out.shape.as_list()[1],DNCON4_1D_out.shape.as_list()[1],contact_feature_num_2D)
#     contact_input = Input(shape=contact_input_shape)   
#     ######################### now merge new data to new architecture
...
def DeepCovAttBefore_SS_with_paras(win_array,feature_num,use_bias,hidden_type,filterss,nb_layers,opt):
    #### for model 40~100
    #feature_num = 40
    #ktop_node= ktop_node
    print("Setting hidden models as ",hidden_type)
    ...
    for fsz in filter_sizes:
        ...
        DeppSS_conv = _attention_layer(feature_num)(DNCON4_input)
        for i in range(0,nb_layers):
            DNCON4_conv = _conv_bn_relu1D(filters=filterss, kernel_size=fsz, subsample=1,use_bias=use_bias)(DNCON4_conv)
            
        # DeppSS_conv = _attention_layer(filterss)(DNCON4_conv)  
        DNCON4_conv = _conv_bn_softmax1D(filters=1, kernel_size=fsz, subsample=1,use_bias=use_bias,name='local_start')(DNCON4_conv)
        #DNCON4_conv = remove_1d_padding(ktop=ktop_node)(DNCON4_conv) ## remove the padding rows because they don't have targets
...
def DeepCovAttAfter_SS_with_paras(win_array,feature_num,use_bias,hidden_type,filterss,nb_layers,opt):
    ...
    for fsz in filter_sizes:
        DNCON4_conv = DNCON4_input
        for i in range(0,nb_layers):
            DNCON4_conv = _conv_relu1D(filters=filterss, kernel_size=fsz, subsample=1,use_bias=use_bias)(DNCON4_conv)
...
def identity_Block_deep_noBN(input, filters, kernel_size, with_conv_shortcut=False,use_bias=True, mode='sum'):
    x = _conv_relu1D(filters=filters/4, kernel_size=1, subsample=1,use_bias=use_bias)(input)
    x = _conv_relu1D(filters=filters/4, kernel_size=kernel_size, subsample=1,use_bias=use_bias)(x)
...
def DeepResnet_with_paras(win_array,feature_1D_num,feature_2D_num,sequence_length,use_bias,hidden_type,filterss,nb_layers,opt,batch_size):
    ...
    for fsz in filter_sizes:
        ...
        DNCON4_1D_conv = Dropout(0.1)(DNCON4_1D_conv)
        DNCON4_1D_conv = BatchNormalization(axis=-1)(DNCON4_1D_conv)
...
def DeepInception_with_paras(win_array,feature_1D_num,feature_2D_num,sequence_length,use_bias,hidden_type,filterss,nb_layers,opt,batch_size):
    ...
    for fsz in filter_sizes:
        ...
        branch_1 = _conv_bn_relu1D(filters=filterss*2, kernel_size=fsz, subsample=1, use_bias=use_bias)(DNCON4_1D_conv)
        ...
        branch_1 = _conv_bn_relu1D(filters=filterss, kernel_size=fsz, subsample=1, use_bias=use_bias)(DNCON4_1D_conv)
...
def _residual_block(filters, kernel_size, use_bias):
    def f(input):
        ...
        return x
    return f
def _attention_block(filters, kernel_size, use_bias, encoder_depth=1):
    def f(input):
        p = 1
        t = 2
        r = 1
        ...
        for i in range(p):
            inputs = _residual_block(filters,kernel_size, use_bias)(input)
        # Trunc Branch
        output_trunk = inputs
        for i in range(t):
            output_trunk = _residual_block(filters,kernel_size, use_bias)(output_trunk)
        # Soft Mask Branch
        ## encoder
        # output_soft_mask = MaxPooling1D(2, padding='same')(inputs)
        for i in range(r):
            output_soft_mask = _residual_block(filters,kernel_size, use_bias)(inputs)
        skip_connections = []
        for i in range(encoder_depth - 1):
            ## skip connections
            output_skip_connection = _residual_block(filters,kernel_size, use_bias)(output_soft_mask)
            skip_connections.append(output_skip_connection)
            # output_soft_mask = MaxPooling1D(2, padding='same')(output_soft_mask)
            for _ in range(r):
                output_soft_mask = _residual_block(filters,kernel_size, use_bias)(output_soft_mask)
        ## decoder
        ...
        for i in range(encoder_depth - 1):
            ...
            for _ in range(r):
                output_soft_mask = _residual_block(filters,kernel_size, use_bias)(output_soft_mask)
            # output_soft_mask = UpSampling1D()(output_soft_mask)
            ## skip connections
            output_soft_mask = merge([output_soft_mask, skip_connections[i]], mode='sum')
        ### last upsampling
        for i in range(r):
            output_soft_mask = _residual_block(filters,kernel_size, use_bias)(output_soft_mask)
        # output_soft_mask = UpSampling1D()(output_soft_mask)
        ## Output
        output_soft_mask = _conv_bn_relu1D(filters=filters, kernel_size=kernel_size, subsample=1, use_bias=use_bias)(output_soft_mask)
        # Attention: (1 + output_soft_mask) * output_trunk
        output = Lambda(lambda x: x + 1)(output_soft_mask)
        output = merge([output, output_trunk], mode='mul')
        # Last Residual Block
        for i in range(p):
            output = _residual_block(filters,kernel_size, use_bias)(output)
        return output
    return f
def DeepCovResAtt_SS_with_paras(win_array, feature_num, use_bias, hidden_type, filterss, nb_layers, opt):
    ...
    for fsz in filter_sizes:
        ...
        DNCON4_conv = _conv_bn_relu1D(filters=filterss, kernel_size=fsz, subsample=1, use_bias=use_bias)(DNCON4_conv)
        for i in range(0, nb_layers):
            DNCON4_conv = _residual_block(filters=filterss, kernel_size=fsz, use_bias=use_bias)(DNCON4_conv)
            # DNCON4_conv = Dropout(0.15)(DNCON4_conv)
            DNCON4_conv = _attention_block(filters=filterss, kernel_size=fsz, use_bias=use_bias, encoder_depth=1)(DNCON4_conv)

        DNCON4_conv = _conv_bn_softmax1D(filters=1, kernel_size=fsz, subsample=1, use_bias=use_bias, name='local_start')(
            DNCON4_conv)
...
def DeepFracNet_with_paras(win_array,feature_1D_num,feature_2D_num,sequence_length,use_bias,hidden_type,filterss,nb_layers,opt,batch_size):
    ...
    for fsz in filter_sizes:
        ...
        for i in range(0, nb_layers):
            ...
            DNCON4_1D_conv = BatchNormalization(axis=-1)(DNCON4_1D_conv)
            DNCON4_1D_conv = Dropout(0.35)(DNCON4_1D_conv)
    ...
    for fsz in filter_sizes:
        ...
        for i in range(0, nb_layers):
            ...
            DNCON4_2D_conv = BatchNormalization(axis=-1)(DNCON4_2D_conv)
            DNCON4_2D_conv = Dropout(0.35)(DNCON4_2D_conv)
