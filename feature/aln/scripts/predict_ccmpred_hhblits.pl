...
foreach my $c (keys %COVERAGE){
	my $hhbid = "hhb-cov".$c;
  system_cmd("rm -f ${hhbid}_ccmpred*");
	next if(-f "$hhbid.ccmpred");
	open  JOB, ">${hhbid}_ccmpred.sh" or confess "ERROR! Could not open ${hhbid}_ccmpred.sh $!";
  print JOB "#!/bin/bash\n";
  print JOB "touch ${hhbid}_ccmpred.running\n";
  print JOB "echo \"running ccmpred ..\"\n";
  print JOB CCMPRED." -t ".NPROC." $hhbid.aln $hhbid.ccmpred > ccmpred.log\n";
  print JOB "if [ -s \"$hhbid.ccmpred\" ]; then\n";
  print JOB "   mv ${hhbid}_ccmpred.running ${hhbid}_ccmpred.done\n";
  print JOB "   echo \"ccmpred job done.\"\n";
  print JOB "   exit\n";
  print JOB "fi\n";
  print JOB "echo \"ccmpred failed!\"\n";
  print JOB "mv ${hhbid}_ccmpred.running ${hhbid}_ccmpred.failed\n";
  close JOB;
	system_cmd("chmod 755 ${hhbid}_ccmpred.sh");
	$jobs{"${hhbid}_ccmpred.sh"} = 1;
}
...
foreach my $c (keys %COVERAGE){
  my $hhbid = "hhb-cov".$c;
  ccmpred2rr("$id.fasta", "$hhbid.ccmpred", "$hhbid.ccmpred.rr", 6);
  system_cmd("cp $hhbid.ccmpred.rr predictions/.");
  system_cmd("rm -f $hhbid.ccmpred*");
}
