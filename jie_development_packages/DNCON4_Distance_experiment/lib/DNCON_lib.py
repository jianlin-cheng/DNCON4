...
def load_sample_data_2D(data_list, path_of_X, path_of_Y,Interval,seq_end, min_seq_sep,dist_string, reject_fea_file='None'):
  ...
  for i in range(0,len(sample_name)):
    ...
    ### load the data
    (featuredata,feature_index_all_dict) = getX_2D_format(featurefile, cov, plm, distpred, accept_list, pdb_lens, notxt_flag)
    # print("\n######",len(featuredata))
...
def get_y_from_this_list_dist_error(selected_ids, path, min_seq_sep, l_max):
  ...
  for pdb in sorted(selected_ids):
    y1  = getY(path + pdb + '.txt', min_seq_sep, l_max)
    #y2  = getY(path + pdb + '-DistError.txt', min_seq_sep, l_max)
    y2  = getY('/storage/htc/bdm/DNCON4/data/cullpdb_dataset/distance_prediction20190315/Error_distance/' + pdb + '-DistError.txt', min_seq_sep, l_max)
...
def get_x_1D_2D_from_this_list(selected_ids, feature_dir, l_max,dist_string, reject_fea_file='None'):
  ...
  for pdb in selected_ids:
    sample_pdb = pdb
    break
  featurefile = '/storage/htc/bdm/DNCON4/feature/other/output/'+sample_pdb+'/feat-'+sample_pdb+'.txt'
...
def getX_2D_format(feature_file, cov, plm, distpred, accept_list, pdb_len = 0, notxt_flag = True):
  # calcualte the length of the protein (the first feature)
  ...
  if '# plm' in accept_list:  
      ...
      for i in range(441):
          ...
          if feature_index in feature_all_dict:
              ...
              feature_all_dict[feature_index] = feature2D
  if '# dist error' in accept_list:
      distpred_raw=np.loadtxt(distpred)
      length = distpred_raw.shape[0]
      chn = 1
      if length != L:
          print("Net Bad Alignment, pls check!")
          sys.exit()
      inputs_distpred =  distpred_raw.reshape(1, chn, L, L)
      for i in range(chn):
          feature2D = inputs_distpred[0][i]
          feature_namenew = '# Distance prediction '+str(i+1)+ ' 2D'
          feature_index +=1
          if feature_index in feature_index_all_dict:
              print("Duplicate feature name ",feature_namenew, " in file ",feature_file)
              sys.exit()
...
def get_x_2D_from_this_list(selected_ids, feature_dir, l_max,dist_string, reject_fea_file='None', pdb_len = 0):
  ...
  plm = '/storage/htc/bdm/DNCON4/feature/plm/cull_plm/output/' + sample_pdb + '.plm'
...
def get_x_2D_from_this_list_useDistance(selected_ids, feature_dir, l_max,dist_string,distpred_dir, reject_fea_file='None', pdb_len = 0):
  xcount = len(selected_ids)
  sample_pdb = ''
  for pdb in selected_ids:
    sample_pdb = pdb
  ...
  #plm =feature_dir + '/'  + sample_pdb + '.plm'
  plm = '/storage/htc/bdm/DNCON4/feature/plm/cull_plm/output/' + sample_pdb + '.plm'
  distpred = distpred_dir +'/' + sample_pdb + '-distance.txt'
  #print(featurefile)
  notxt_flag = True
  if ((len(accept_list) == 1 and ('# cov' not in accept_list and '# plm' not in accept_list)) or 
        (len(accept_list) == 2 and ('# cov' not in accept_list or '# plm' not in accept_list and '# dist error' not in accept_list)) or (len(accept_list) > 2)):
  ...
  for pdb_name in sorted(selected_ids):
      ...
      if '# plm' in accept_list:
        if not os.path.isfile(plm):
                    print("plm matrix file not exists: ",plm, " pass!")
                    continue 
      distpred = distpred_dir +'/' + pdb_name + '-distance.txt'
      if '# dist error' in accept_list:      
        if not os.path.isfile(distpred):
                    print("dist error file not exists: ",distpred, " pass!")
                    continue 
