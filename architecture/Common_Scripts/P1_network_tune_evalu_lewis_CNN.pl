...
for($i=0;$i<=8;$i+=1)
{
  ...
  # @weights=(5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 100, 150, 200);
  # $w_seed=int(rand(13));
  # $weight=$weights[$w_seed];
  # $batchsize = 3;
  ...
  # $w_seed=int(rand(13));
  $weight=$weights[$i]; 
  # @Activate_M=('\'linear\'','\'elu\'','\'softplus\'','\'softsign\'','\'relu\'','\'tanh\'','\'sigmoid\'','\'hard_sigmoid\'');
  $activate='\'relu\'';
  print "filternum $filternum; layernum $layernum; kernelsize $kernelsize; batchsize $batchsize; initializer $initializer; loss_function $loss_function; weight $weight; activate $activate\n";
  #INCEP
  # $filternum=int(32);
  # $layernum=int(5);
  # $kernelsize=int(4);
  ...
  if($netname eq "DNCON4_1d2dCNN" ){  
  ...
  elsif($netname eq "DNCON4_2dCNN_Z" ){  
    print SH "python \$gloable_dir/architecture/CNN_arch/scripts/train_deepCNN_2D_gen_tune.py 150 $filternum  $layernum 'nadam' $kernelsize  30 1 \$feature_dir \$output_dir \$acclog_dir $batchsize $initializer $loss_function $weight $activate\n";
  }
    ...
    print SH "python \$gloable_dir/architecture/ResNet_arch/scripts/train_deepResNet_2D_gen_tune.py 150 $filternum  $layernum 'nadam' $kernelsize  30 5 \$feature_dir \$output_dir \$acclog_dir $batchsize $initializer $loss_function $weight\n";
  }
