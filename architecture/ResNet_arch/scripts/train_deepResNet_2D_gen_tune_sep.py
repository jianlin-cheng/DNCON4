...
if not os.path.exists(CV_dir):
  ...
  if rerun_epoch <= 0:
    ...
    sys.exit(1)
  print("####### Restart at epoch %i of epoch %i" %(rerun_epoch, out_epoch))
...
data_all_dict_padding = load_train_test_data_padding_with_interval_2D(sample_datafile, feature_dir,inter,5000,0,dist_string, reject_fea_file, sample_flag=True)
...
best_acc=DNCON4_1d2dconv_train_win_filter_layer_opt_fast_2D_generator(data_all_dict_padding,CV_dir, feature_dir,"DNCON4_2dRES",out_epoch,in_epoch,rerun_epoch,inter,
  ...
  initializer, loss_function, weight_p, list_sep_flag=True)
