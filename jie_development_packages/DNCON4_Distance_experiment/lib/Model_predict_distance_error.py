...
# python /scratch/jh7x3/DNCON4/jie_development_packages/DNCON4_Distance_experiment/lib/Model_predict_distance_error.py ../model-train-DNCON4_2dRES.json  ../model-train-weight-DNCON4_2dRES-best-val.h5 ./evadir testlist 
...
reject_fea_file = '/scratch/jh7x3/DNCON4/jie_development_packages/DNCON4_Distance_experiment/lib/feature_to_use_cov_disterror.txt'
...
for key in selected_list:
    ...
    other_cmap_error_file = "%s/%s-error.txt" % (model_predict, key)
    if os.path.isfile(other_cmap_distance_file):
        print("Already exist: ",other_cmap_distance_file)
        continue
    selected_list_2D_other = get_x_2D_from_this_list_useDistance(p1, path_of_X, Maximum_length,dist_string,distpred_dir, OTHER, value)
    if type(selected_list_2D_other) == bool:
            continue
    DNCON4_prediction_other = DNCON4.predict([selected_list_2D_other], batch_size= 1)
     
    DNCON4_CNN_prediction_disterror = np.copy(DNCON4_prediction_other)
    DNCON4_CNN_prediction_dist = DNCON4_prediction_other[:,:,:,0]
    DNCON4_CNN_prediction_error = DNCON4_prediction_other[:,:,:,1]
    ...
    DNCON4_CNN_prediction_error = real_cmap_error.reshape(Maximum_length, Maximum_length)
    ...
    DNCON4_CNN_prediction_contact[DNCON4_CNN_prediction_contact>1]=0
    #other_cmap_distance_file = "%s/%s-distance.txt" % (model_predict, key)
    #other_cmap_contact_file = "%s/%s-contact.txt" % (model_predict, key)
    ...
    np.savetxt(other_cmap_error_file, DNCON4_CNN_prediction_error, fmt='%.4f')
