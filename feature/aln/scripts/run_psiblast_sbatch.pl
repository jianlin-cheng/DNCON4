...
foreach $file (@files)
{
  ...
  $runfile="$sbatch_folder/$targetid-psiblast.sh";
  ...
  print SH "echo \" perl /storage/htc/bdm/DNCON4/feature/aln/scripts/generate_psiblast_aln.pl $fastafile $nr_db  $outputdir/$targetid &> $outputdir/$targetid.log\"\n\n";

  print SH "SECONDS=0\n\n";

  print SH "perl /storage/htc/bdm/DNCON4/feature/aln/scripts/generate_psiblast_aln.pl $fastafile $nr_db  $outputdir/$targetid &> $outputdir/$targetid.log\n\n";
